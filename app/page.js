...
export default function SED() {
  ...
  const handleAction = async (req, actionType, payload = {}) => {
      ...
      if (actionType === 'TOGGLE_URGENCY') { updates.urgency = payload.isUrgent ? null : 'срочно'; }
          ...
          if (actionType === 'REJECT') {
              ...
              if (!reason) return; 
              updates.step_komer = 0; updates.status = "ОТМЕНЕНО КОМЕРОМ"; updates.fix_comment = "Отказ Ком. Директора: " + reason; comments = reason;
          }
      }
  ...
  const handleComingSoon = (e) => {
    e.preventDefault();
    alert("🚀 По техническим причинам новая версия СЭД v2.0 начнет работу 13 марта в 05:00.\nК этому времени мы уже всё исправим! 😉");
  };
